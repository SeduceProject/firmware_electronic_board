    ...
    with open(HEX) as f:
        ...
        if (handshakeResponse == HANDSHAKE_OK):
            ...
            while seq < len(content):
                ...
                isEOF = (
                content[seq].strip() == ":00000001FF")  # this should be the last line in any valid intel HEX file
                result = -1
                bundledLines = 1
                ...
                if isEOF == False:
                    ...
                    if LINESPERPACKET > 1 and currentLine[1:3] == '10' and len(currentLine) == 43:
                        ...
                        if nextLine != ":00000001FF" and nextLine[1:3] == "10" and len(nextLine) == 43:
                            ...
                            checksum = int(currentLine[41:43], 16) + int(nextLine[41:43], 16) + int(nextLine[3:5],
                                                                                                    16) + int(
                                nextLine[5:7], 16)

                            # check if a third line != EOF, so we can bundle 3 lines
                            nextLine2 = content[seq + 2].strip()
                            if LINESPERPACKET == 3 and nextLine2 != ":00000001FF" and nextLine2[1:3] == "10" and len(
                                    nextLine2) == 43:
                                # {
                                # need to sum: the previous checksum + address bytes of nextLine2 (to arrive at a correct final checksum of combined 3 lines
                                checksum += int(nextLine2[41:43], 16) + int(nextLine2[3:5], 16) + int(nextLine2[5:7],
                                                                                                      16)
                                hexDataToSend = ":3" + hexDataToSend[2:(len(hexDataToSend) - 2)] + nextLine[
                                                                                                   9:41] + nextLine2[
                                                                                                           9:41] + (
                                                '%0*X' % (2, checksum % 256))
                                bundledLines = 3
                            # }
                                ...
                                # {
                                hexDataToSend = ":2" + hexDataToSend[2:(len(hexDataToSend) - 2)] + nextLine[9:41] + (
                                '%0*X' % (2, checksum % 256))
                                bundledLines = 2;
                                # }
                                # }
            ...
            print "FAIL: No response from Moteino programmer, is it connected to " + SERIALPORT
            exit(1)
        elif (handshakeResponse == HANDSHAKE_FAIL_TIMEOUT):
            print "FAIL: No response from Moteino programmer, is it connected to " + SERIALPORT
            exit(1)
